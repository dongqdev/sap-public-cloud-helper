...
`icons/icon128.png` (128×128, 실제 사이즈로 새로 생성됨) 업로드.
...
```
이 확장 프로그램의 모든 JavaScript는 패키지 안에 포함되어 있으며, 런타임에 외부 서버에서
코드를 내려받거나 eval()로 실행하지 않습니다. 원격 서버로의 네트워크 요청 자체가 없습니다.
```

## 데이터 사용 인증

개인정보 보호 관행 탭의 데이터 수집 항목(개인 식별 정보, 위치, 검색 기록 등)은 전부 "아니요"로
표시하면 됩니다. `privacy.html`에 정리한 대로 자체 서버가 없고 외부로 전송하는 데이터가 없으므로,
"개발자 프로그램 정책을 준수한다"는 인증 체크박스를 그대로 체크하면 됩니다.
...
- `images/image 7.png` (라이트 모드), `images/image 8.png` (다크 모드) — 실제 popup UI를
  샘플 데이터로 채워서 브라우저 프레임 + 기능 소개 문구와 합성한 1280×800 마케팅 스크린샷
