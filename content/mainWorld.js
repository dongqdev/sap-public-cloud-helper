...
(() => {
  ...
  class FioriUi5AutomationEngine {
    ...
    async waitForUi5(timeout = 8000) {
      ...
      while (Date.now() - startTime < timeout) {
        ...
        if (typeof sap !== 'undefined' && sap.ui && sap.ui.core && sap.ui.core.BusyIndicator) {
          if (typeof sap.ui.core.BusyIndicator.isBusy === 'function' && sap.ui.core.BusyIndicator.isBusy()) {
            isBusy = true;
          }
        }
        ...
        if (!isBusy && typeof sap !== 'undefined' && sap.ui && sap.ui.getCore) {
          ...
          if (core) {
            ...
            // 최상위 컨테이너 Busy 체크
            if (!isBusy) {
              const appContainer = document.querySelector('.sapUShellShell, .sapMApp, .sapUComponentContainer');
        ...
        if (!isBusy && typeof sap !== 'undefined' && sap.ui && sap.ui.getCore) {
          const core = sap.ui.getCore();
          if (core) {
            const models = (core.getComponent && core.getComponent() && core.getComponent().getModels) 
                           ? core.getComponent().getModels() 
                           : (core.mModels || {});
            
            for (const name in models) {
              const model = models[name];
              if (model && typeof model.hasPendingRequests === 'function' && model.hasPendingRequests()) {
                isBusy = true;
                break;
              }
            }
        ...
        while (Date.now() - searchMboxStartTime < 4000) {
          ...
          for (const dlg of dialogs) {
            ...
            for (const btn of btns) {
              ...
              if (txt.includes('확인') || txt.includes('OK') || txt.includes('YES') || txt.includes('예')) {
    ...
    async processBusinessRole() {
      ...
      const isAlreadyInDetails = window.location.hash.includes('maintainNew') && 
                                 !this.findControl('btnFooterMainAction') && 
                                 this.findControl('iconTabFilter.BusinessCatalog');

      if (isAlreadyInDetails) {
        this.log("[Role] 이미 기존 역할의 상세 편집 화면에 수동으로 진입해 있는 상태를 감지했습니다.");
        ...
        if (roleIdInput && roleDescInput) {
          ...
          if (isDuplicate) {
            this.log("[Role] 중복 역할 감지 (예약된 ID 또는 생성 비활성화). 다이얼로그를 취소하고 메인 화면에서 검색 후 자동화를 조기 종료합니다.", 'error');
      ...
      if (editBtn) {
        ...
        if (isVisible && isEnabled) {
          this.log("[Role] 기존에 저장 완료된 역할 상태이므로 '편집' 버튼을 클릭하여 수정 모드로 진입합니다.");
          editBtn.firePress();
          await this.waitForUi5();
          await this.delay(500);
        }
      ...
      if (catalogId) {
        const catalogIds = catalogId.split(',').map(s => s.trim()).filter(Boolean);
        ...
        if (addBtn) {
          ...
          for (const targetCatalogId of catalogIds) {
            const searchInput = await this.waitForControl('searchField.AddBusinessCatalogAssignmentTable');
            if (searchInput) {
              ...
              const table = this.findControl('table.AddBusinessCatalogAssignment') || this.findControl('AddBusinessCatalogAssignmentTable');
              if (table) {
                // 테이블 바인딩 완료 대기 (최대 5초)
                ...
                for (const item of items) {
                  ...
                  for (const cell of cells) {
                    ...
                    if (text.trim() === targetCatalogId || title.trim() === targetCatalogId || 
                        text.indexOf(targetCatalogId) !== -1 || title.indexOf(targetCatalogId) !== -1) {
                      isExactMatch = true;
                      break;
                    }
                  }
                ...
                if (targetCheck) {
                  ...
                  const registry = (sap.ui.core.Element && sap.ui.core.Element.registry) || (core && core.mElements);
                  if (registry) {
                    const allItems = registry.all ? registry.all() : registry;
                    const keys = Object.keys(allItems);
                    for (const key of keys) {
                  ...
                  if (sap.m && sap.m.InstanceManager) {
                    ...
                    for (const dlg of dialogs) {
                      ...
                      if (title.indexOf('종속') !== -1 || title.toUpperCase().indexOf('DEPENDENCY') !== -1) {
                        dependencyDlg = dlg;
                        break;
                      }
                    }
                  ...
                  if (dependencyDlg) {
                    this.log("[Role] 카탈로그 종속성 다이얼로그가 감지되었습니다. 종속성은 체크 제외하고 즉시 적용합니다.");
                    ...
                    for (const btn of buttons) {
                      ...
                      if (txt.includes('적용') || txt.includes('확인') || txt.includes('OK') || txt.includes('APPLY')) {
                  ...
                  this.log(`[경고] 일치하는 비즈니스 카탈로그(${targetCatalogId})를 테이블에서 찾지 못했습니다.`, 'warning');
                }
              }
            }
          }
  ...
  function runProfileInformationInjector() {
    ...
    if (typeof sap === 'undefined' || !sap.ui || !sap.ui.getCore || !sap.ushell || !sap.ushell.Container) {
      setTimeout(runProfileInformationInjector, 500);
      return;
    }

    // Fiori Shell UI 화면이 실제로 그려질 때까지 대기 (화면 생성 완료 대기)
    const shellHeader = document.getElementById('shell-header') || document.querySelector('.sapUShellShell');
    if (!shellHeader) {
      setTimeout(runProfileInformationInjector, 500);
      return;
    }
    ...
    setInterval(() => {
      ...
      let infoBar = document.querySelector('.sapUshellSystemInfoBar, .sapUshellSysInfoBar, [id*="SystemInfoBar"], [id*="SysInfoBar"], [class*="SystemInfoBar"], [class*="SysInfoBar"]');
      if (!infoBar) {
          ...
          if (!infoTrigger) {
            ...
            infoTrigger.addEventListener('click', (e) => {
              ...
              function clickTarget(aboutDom) {
                if (!aboutDom) return;
                const innerTarget = aboutDom.shadowRoot ? aboutDom.shadowRoot.querySelector('li') : null;
                if (innerTarget) {
                  innerTarget.click();
                } else {
                  setTimeout(() => {
                    const retryTarget = aboutDom.shadowRoot ? aboutDom.shadowRoot.querySelector('li') : aboutDom;
                    retryTarget?.click();
                  }, 50);
                }
              }
              ...
              const profileDom = document.querySelector('#meAreaHeaderButton') || 
      ...
      const popovers = document.querySelectorAll('ui5-popover, ui5-responsive-popover, .sapMPopover, [id*="popover"]');
      for (const popover of popovers) {
        const isVisible = popover.hasAttribute('open') || 
                          popover.classList.contains('sapMPopoverOpen') || 
                          (popover.style && popover.style.display !== 'none' && popover.style.visibility !== 'hidden');
        if (isVisible) {
  ...
  function replaceProfileNameInContainer(root, targetName, newText) {
    ...
    while (queue.length > 0) {
      ...
      // 2. 일반 텍스트 노드 매칭 및 치환
      if (node.children && node.children.length === 0 && (!node.shadowRoot || node.shadowRoot.children.length === 0)) {
        if (node.textContent && node.textContent.trim() === targetName && node.textContent !== newText) {
